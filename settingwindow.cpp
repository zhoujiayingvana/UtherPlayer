    ...
    ui(new Ui::SettingWindow),media(m)
{
    ...
    ui->page2ScrollAreaWidgetContents->setMinimumSize(0,1170);
    //设置滚动条不显示,用自定义的滚动条
    ...
    //scrollBar->setStyleSheet(qss1);
    scrollBar->setVisible(true);
    connect(ui->scrollArea_2->verticalScrollBar(), SIGNAL(valueChanged(int)), scrollBar, SLOT(setValue(int)));
    connect(scrollBar, SIGNAL(valueChanged(int)), ui->scrollArea_2->verticalScrollBar(), SLOT(setValue(int)));
    connect(ui->scrollArea_2->verticalScrollBar(), SIGNAL(rangeChanged(int, int)), scrollBar, SLOT(setRange(int, int)));
...
/**
* @method        SettingWindow::on_quickMoveSpinBox_valueChanged
* @brief         修改快进快退时间
* @param         INT
* @return        VOID
* @author        涂晴昊
